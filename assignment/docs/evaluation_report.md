...
Early-Warning setup (no grades, available before any assessment exists): development recall 0.482. Confirmatory (G1+G2) setup: development recall 0.904. This is a limitation observed in our current experiments, not a proven irreducible limit: Early-Warning mode predicts before any grade exists, using only attendance, study habits, and past failures, so weaker results here are expected given the narrower feature set -- but these results do not prove that better Early-Warning performance is impossible with more data or features. SARAH defaults to Early-Warning mode regardless, since catching risk earlier -- even less accurately -- is the point of an early-warning system; Confirmatory mode is available as a second look once grades exist.
