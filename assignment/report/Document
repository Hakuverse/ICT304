...
mode/model/weight setting, plus a confusion matrix for the chosen option.]
...
as planned. The current #26 work prepares inputs, not a trained AI prototype.]
...
| T03 | Confirmatory inputs | Same inputs plus previous score; no G3 | Return exactly four inputs | Benjamin | Automated check included with #26 |
...
| T07 | Missing or invalid input | Missing study hours, text instead of a number, attendance of 150, unknown mode | Stop and explain the input error rather than guessing a value | Benjamin; interface tester to confirm later | Helper check included; screen behavior planned |
| T08 | Model comparison | Logistic Regression and Decision Tree, both modes, both weight settings | Use the same five groups of students and report accuracy, High-Risk precision, recall and F1 | Prototype owner to confirm | Sprint 4; follow #18 and #19 |
...
| T12 | CSV errors | Empty CSV, missing required column, blank cell and invalid value | Explain the error and do not score invalid data. Agree whole-file rejection or row-level errors before building upload | Benjamin proposed; team to confirm | Planned; file-upload behavior not built |
...
| System engineering phases and references | Anna, based on #20 | Review her PR #37 and agree one phase mapping for the whole report |
| Architecture diagram | Jackie, based on #17 | Review PR #41 and check exact mode inputs |
| System design and teacher workflow | Anna, based on #21 | Review PR #39 and align its feature list and implementation claims with the code |
| Model evaluation method | Jackie, based on #18 | Review PR #40; use the same student groups in each comparison |
| Class weights and comparison plan | Anna, based on #19 | Review PR #38; link reproducible code before treating its numbers as final results |
...
| AI techniques, prototype code, results and limitations | Team to choose a prototype lead | Agree ownership of #30-#33; keep both techniques in the investigation |
...
- [ ] Choose the prototype lead and the person checking their run: ____ / ____.
- [ ] Agree a first working model run. Suggested target: 28 September; confirm: ____.
...
- [ ] Setup/run instructions tried by another teammate. The README currently covers
  data preparation; add the actual training command when the prototype exists.
...
data preparation. Then run the input tests above. The [mode guide](../docs/mode-guide.md)
explains which function the later training code should use. Record the teammate's
